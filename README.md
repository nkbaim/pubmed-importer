...
- 分页浏览题名、作者、期刊、日期、PMID 和 DOI；摘要默认预览三行，可展开全文
- 选择当前页或跨页选择多篇文献后批量导入
- 导入到当前 Zotero 文库或选中的分类
- 自动识别当前文库内已有的 PMID，避免重复导入
- 使用 Zotero 内置 PMID 翻译器保存规范题录
- 导入后自动调用 Zotero 原生“查找可用全文”，尝试保存可获取的 PDF
- 已存在和刚导入的记录在“已存在”标签下方显示紫色 `Zotero URL` 文本链接，点击即可复制完整的 `zotero://select/...` 地址
- 适配浅色和深色界面
...
- 全文查找由 Zotero 原生机制完成，能否获取 PDF 取决于开放获取状态、机构订阅和 Zotero 的全文解析能力。
